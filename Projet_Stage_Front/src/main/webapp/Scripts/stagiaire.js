...
function updateTable() {
    $.ajax({
        url: 'http://localhost:8080/Projet_Stage_Front/StagiaireServlet?action=getAll',
        method: 'GET',
...
function updateTable1() {
    $.ajax({
        url: 'http://localhost:8080/Projet_Stage_Front/StagiaireServlet?action=SansStage',
        method: 'GET',
        success: function(response) {
            var tableContent = $(response).find('#tableContent1').html();
            $('#tableContent1').html(tableContent);
            console.log(response);
        },
        error: function(xhr, status, error) {
            console.error(error);
